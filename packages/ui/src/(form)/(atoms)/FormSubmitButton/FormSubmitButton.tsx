...
interface Props {
  ...
  /**
   * Error message, if the form submission failed.
   * Issues with specific inputs should be handled by the input itself.
   */
  error?: string
}
...
export default function FormSubmitButton(props: Props) {
  ...
  const Spinner = () => (
    <svg
      aria-hidden="true"
      className="absolute -left-4 -top-4 mr-2 h-8 w-8 animate-spin fill-blue-700 text-gray-300"
      viewBox="0 0 100 101"
      fill="none"
    ...
    >
      <path
        d={
          'M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 ' +
      ...
      <path
        d={
          'M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 ' +
    ...
    <div className="relative flex w-full flex-col gap-1 pl-2">
      <button
        className={classNames(
          'relative mt-10 w-full gap-6 rounded-lg bg-action-primary px-6 py-3 font-semibold tracking-wide text-on-secondary',
      ...
      >
        {props.children}
        <div className={classNames('absolute right-6 top-1/2', { hidden: !props.loading })}>
          <Spinner />
        </div>
      </button>
